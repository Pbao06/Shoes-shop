...
import { Award, Plus, Search, Trash2, Pencil, RefreshCcw } from 'lucide-react';
import Modal from '@/components/ui/Modal';
import type { AdminBrand } from '@/types/admin/products';

export default function AdminBrandsPage() {
  const { brands, loading, error, refetch } = useAdminBrands();
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      ...
      showToast('Failed to delete brand. It may contain products.', 'error');
    } finally {
      setDeletingId(null);
    }
  };
  ...
  if (error) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center px-6 py-16">
        <p className="font-serif text-2xl text-[#1a1714]">Failed to load brands</p>
  ...
  return (
    <div className="px-6 py-10 md:px-12 md:py-16">
      <div className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="font-serif text-3xl tracking-[-0.03em] md:text-4xl">Brands</h1>
      ...
      <div className="border border-[#1a1714]/10 bg-[#fcfbf8]">
        <div className="border-b border-[#1a1714]/10 px-6 py-5 md:px-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="font-serif text-xl tracking-[-0.02em]">All Brands</h2>
            ...
            <table className="w-full text-left text-[13px]">
              ...
              <tbody className="divide-y divide-[#1a1714]/10">
                {filteredBrands.map((brand) => (
                  <tr key={brand.id} className="transition-colors hover:bg-[#1a1714]/[0.02]">
                    <td className="px-6 py-4 md:px-8">
                      <p className="font-medium text-[#1a1714]">{brand.name}</p>
                    </td>
                    ...
                    <td className="px-4 py-4 md:px-8">
                      <div className="flex items-center gap-1">
                        <BrandFormTrigger brand={brand} />
                        <button
                          type="button"
                          onClick={() => setDeleteModal({ open: true, id: brand.id, name: brand.name })}
...
function BrandFormTrigger({ brand }: { brand?: AdminBrand }) {
  ...
  return (
    <>
      ...
      <BrandFormModal
        ...
        onSuccess={() => {
          setIsOpen(false);
          showToast(brand ? 'Brand updated successfully' : 'Brand created successfully');
        }}
      />
    </>
  );
...
function BrandFormModal({
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      if (isEditing && brand) {
        await adminBrandService.update(brand.id, payload);
      } else {
