...
export default function CartPage({ userId = DEFAULT_USER_ID }: CartPageProps) {
  const router = useRouter()
  const { items, loading, error, totalItems, totalPrice, updateItem, removeItem, refetch } = useCartApi(userId)
  const { user } = useAuth()
        ...
        <div className="mx-auto max-w-[1400px] px-5 pb-24 pt-6 md:px-10 md:pt-10 lg:px-16">
          <header className="pb-10 md:pb-14">
            ...
            <div className="mt-5 flex items-start justify-between">
              ...
              <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground pt-2">Loading…</p>
            </div>
          </header>
          <div className="flex min-h-[360px] items-center justify-center">
            <p className="text-[13px] tracking-[0.02em] text-muted-foreground">Loading cart…</p>
          </div>
        </div>
      </main>
    )
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-[1400px] px-5 pb-24 pt-6 md:px-10 md:pt-10 lg:px-16">
        ...
        {error && (
          <div className="mb-6 flex items-center justify-between rounded border border-[#b3261e] bg-[#fef2f2] px-4 py-3 text-[13px] text-[#b3261e]">
            <span>{error}</span>
            <button onClick={() => refetch()} className="ml-4 whitespace-nowrap underline underline-offset-4 hover:opacity-70">Retry</button>
          </div>
        )}
          ...
          <div className="grid gap-16 pt-2 lg:grid-cols-[1fr_360px] lg:gap-24">
            <section aria-labelledby="cart-items-heading">
              ...
              <div className="border-t border-border">
                {items.map((item) => (
                  <article key={item.id} className="grid gap-5 border-b border-border py-7 sm:grid-cols-[150px_1fr] md:grid-cols-[180px_1fr]">
                    <div className="relative aspect-square overflow-hidden bg-secondary">
                      {item.image ? (
                        <Image src={item.image} alt={item.name} fill sizes="180px" className="object-cover" />
                      ) : (
                        <div className="absolute inset-0 bg-[#e9e4da]" aria-hidden="true" />
                      )}
                    </div>
                    <div className="flex min-h-full flex-col justify-between gap-8">
                      <div className="flex items-start justify-between gap-5">
                        ...
                        <p className="shrink-0 text-sm">{formatPrice(item.totalPrice)}</p>
                      </div>
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center border border-border" aria-label={`Quantity for ${item.name}`}>
                          <button type="button" onClick={() => updateItem(item.id, item.quantity - 1)} disabled={item.quantity <= 1} className="h-9 w-9 text-lg text-muted-foreground transition-colors hover:text-foreground disabled:opacity-30" aria-label={`Decrease ${item.name} quantity`}>−</button>
                          <span className="w-8 text-center text-xs" aria-live="polite">{item.quantity}</span>
                          <button type="button" onClick={() => updateItem(item.id, item.quantity + 1)} className="h-9 w-9 text-lg text-muted-foreground transition-colors hover:text-foreground" aria-label={`Increase ${item.name} quantity`}>+</button>
                        </div>
                        <button type="button" onClick={() => removeItem(item.id)} className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground underline-offset-4 hover:text-foreground hover:underline">Remove</button>
                      </div>
                    </div>
                  </article>
                ))}
            ...
            <aside className="h-fit border-t border-border pt-7 lg:sticky lg:top-8" aria-labelledby="summary-heading">
              ...
              <dl className="mt-8 space-y-4 border-b border-border pb-7 text-sm">
                <div className="flex justify-between"><dt className="text-muted-foreground">Subtotal</dt><dd>{formatPrice(totalPrice)}</dd></div>
                ...
                <div className="flex justify-between pt-4 text-base"><dt>Total</dt><dd>{formatPrice(totalPrice + shipping)}</dd></div>
              </dl>
              <button
                type="button"
                onClick={() => {
                ...
                className="mt-7 block w-full bg-foreground px-6 py-4 text-center text-[10px] uppercase tracking-[0.24em] text-background transition-opacity hover:opacity-80"
              >
                Checkout
              </button>
